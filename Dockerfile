...
# No `USER runlog` here, deliberately — docker-entrypoint.sh must run as root first
# (to chown the persistent /data volume, which Docker/an older image may have left
# root-owned) before it drops to runlog itself via `su`. Setting USER at the image
# level would make that chown fail as a non-root user with no CAP_CHOWN (a real
# regression caught in a debug-branch commit that added it without the entrypoint's
# root requirement in mind).

# curl isn't installed in python:3.12-slim, so use urllib instead of adding a
# dependency just for this. /health (SnapDeploy's documented convention — see
# app/main.py; /healthz kept as an alias) has no auth/DB dependency, so this only
# ever reflects whether uvicorn itself is up and serving. Reads $PORT to match
# docker-entrypoint.sh's actual bind port, not the 8000 default, since SnapDeploy
# (or any host) may assign a different port.
